...
def decode_video_frames_torchvision(
    ...
    if not hasattr(torchvision.io, "VideoReader"):
        return decode_video_frames_opencv(video_path, timestamps, tolerance_s, log_loaded_timestamps)
...
def decode_video_frames_opencv(
    ...
    """Fallback video decoder for torchvision builds without io.VideoReader."""
    import cv2
    import numpy as np
    ...
    try:
        ...
        if fps <= 0 or frame_count <= 0:
            raise OSError(f"OpenCV could not read video metadata: {video_path} ({fps=}, {frame_count=})")
        ...
        diff = torch.abs(query_ts - loaded_ts_tensor)
        is_within_tol = diff < tolerance_s
        assert is_within_tol.all(), (
            f"One or several query timestamps unexpectedly violate the tolerance ({diff[~is_within_tol]} > {tolerance_s=})."
            "It means that the closest frame that can be loaded from the video is too far away in time."
            "This might be due to synchronization issues with timestamps during data collection."
            "To be safe, we advise to ignore this item during training."
            f"\nqueried timestamps: {query_ts}"
        ...
        frames = []
        for frame_idx, loaded_timestamp in zip(frame_indices, loaded_ts, strict=True):
            cap.set(cv2.CAP_PROP_POS_FRAMES, frame_idx)
            ok, frame = cap.read()
            if not ok or frame is None:
                raise OSError(f"OpenCV could not read frame {frame_idx} from video: {video_path}")
            if log_loaded_timestamps:
                logging.info(f"frame loaded at timestamp={loaded_timestamp:.4f}")

            frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            frame = np.ascontiguousarray(frame)
            frames.append(torch.from_numpy(frame).permute(2, 0, 1))

        closest_frames = torch.stack(frames).type(torch.float32) / 255
    finally:
        cap.release()
