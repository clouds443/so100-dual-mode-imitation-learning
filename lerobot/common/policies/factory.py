...
def make_policy(
    ...
    参数:
        hydra_cfg: 一个已解析的 Hydra 配置（参见脚本）。如果提供了 `pretrained_policy_name_or_path`，
            则仅使用 `hydra_cfg.policy.name`，而忽略其他所有内容。
        pretrained_policy_name_or_path: Hub 上托管的模型的 repo ID 或包含使用 `Policy.save_pretrained`
            保存的权重的目录的路径。请注意，提供此参数会覆盖 `hydra_cfg.policy` 中除
            `hydra_cfg.policy.name` 之外的所有内容。
        dataset_stats: 用于策略中输入/输出（非）归一化的数据集统计信息。在初始化新策略时必须提供，
            在加载预训练策略时不得提供。因此，此参数与 `pretrained_policy_name_or_path` 互斥。

    返回:
        一个策略实例。

    引发:
        ValueError: 如果 `pretrained_policy_name_or_path` 和 `dataset_stats` 中只有一个被提供。
    """
