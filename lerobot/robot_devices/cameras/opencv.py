...
class OpenCVCamera:
    ...
    def connect(self):
        ...
        camera_idx = f"/dev/video{self.camera_index}" if system == "Linux" else self.camera_index
        video_capture_args = (camera_idx, cv2.CAP_DSHOW) if system == "Windows" else (camera_idx,)
        # First create a temporary camera trying to access `camera_index`,
        # and verify it is a valid camera by calling `isOpened`.
        tmp_camera = cv2.VideoCapture(*video_capture_args)
        ...
        self.camera = cv2.VideoCapture(*video_capture_args)
