...
def make_optimizer_and_scheduler(cfg, policy):
    ...
    此函数检查配置中的策略名称 (`cfg.policy.name`)，并创建相应的优化器和学习率调度器。
    它支持 'act'、'diffusion'、'tdmpc' 和 'vqbet' 等策略的不同配置。
    ...
    返回:
        ...
        - torch.optim.lr_scheduler._LRScheduler or None: 学习率调度器，如果不使用则为 None。

    引发:
        NotImplementedError: 如果配置中的策略名称不受支持。
    """
...
def update_policy(
    ...
    此函数计算损失、执行反向传播、裁剪梯度，并使用优化器更新模型参数。
    它支持自动混合精度 (AMP) 以提高性能。

    参数:
        policy (nn.Module): 要更新的策略模型。
        batch (dict): 包含训练数据的张量字典。
        optimizer (torch.optim.Optimizer): 优化器。
        grad_clip_norm (float): 梯度裁剪的最大范数。
        grad_scaler (GradScaler): 用于 AMP 的梯度缩放器。
        lr_scheduler (torch.optim.lr_scheduler._LRScheduler, optional): 学习率调度器。默认为 None。
        use_amp (bool, optional): 是否使用自动混合精度。默认为 False。
        lock (Lock, optional): 用于安全多线程更新的线程锁。默认为 None。

    返回:
        dict: 包含日志信息的字典，例如损失、梯度范数、学习率和更新时间。
    """
...
def log_train_info(logger: Logger, info, step, cfg, dataset, is_online):
    ...
    此函数格式化关键训练指标并将其记录到控制台和配置的记录器（例如 WandB）。

    参数:
        logger (Logger): 记录器实例。
        info (dict): 包含要从更新步骤中记录的指标的字典。
        step (int): 当前训练步骤。
        cfg (DictConfig): Hydra 配置对象。
        dataset (MultiLeRobotDataset): 训练数据集。
        is_online (bool): 指示训练是在线还是离线的标志。
    """
    loss = info["loss"]
...
def log_eval_info(logger, info, step, cfg, dataset, is_online):
    ...
    此函数格式化关键评估指标并将其记录到控制台和配置的记录器（例如 WandB）。

    参数:
        logger (Logger): 记录器实例。
        info (dict): 包含要记录的指标的字典。
        step (int): 当前训练步骤。
        cfg (DictConfig): Hydra 配置对象。
        dataset (MultiLeRobotDataset): 训练数据集。
        is_online (bool): 指示训练是在线还是离线的标志。
...
def train(cfg: DictConfig, out_dir: str | None = None, job_name: str | None = None):
    ...
    此函数处理训练循环，包括离线和在线阶段。它初始化日志记录、数据集、策略、
    优化器和评估环境。它还处理从检查点恢复训练。

    参数:
        cfg (DictConfig): Hydra 配置对象。
        out_dir (str | None, optional): 保存所有运行输出的目录。默认为 None。
        job_name (str | None, optional): 作业的名称。默认为 None。

    引发:
        NotImplementedError: 如果 `out_dir` 或 `job_name` 为 None，或者在线训练与 `LeRobotMultiDataset` 一起使用。
        RuntimeError: 如果 `resume=True` 但找不到检查点，或者输出目录已存在但未使用 `resume=True`。
        ValueError: 如果评估批次大小大于评估回合数。
    """
    if out_dir is None:
        raise NotImplementedError()
    if job_name is None:
    ...
    def evaluate_and_checkpoint_if_needed(step, is_online):
        ...
        此辅助函数检查是否应在当前训练步骤执行评估或保存检查点。
        它处理日志记录、视频录制和保存模型、优化器和调度器状态。

        参数:
            step (int): 当前训练步骤。
            is_online (bool): 指示训练阶段是在线还是离线的标志。
        """
    ...
    while True:
        ...
        def sample_trajectory_and_update_buffer():
            ...
            此辅助函数使用当前策略在环境中执行一个回合，收集数据，
            然后将收集到的数据添加到在线缓冲区中。它还更新了用于在线训练的采样器权重。

            返回:
                tuple[float, float]: 一个包含在线部署耗时和更新在线缓冲区耗时的元组。
            """
...
def train_cli(cfg: dict):
    ...
    这个函数作为使用 Hydra 进行配置管理的训练入口点。
    它从 Hydra 配置中提取输出目录和作业名称，然后调用主 `train` 函数。
...
def train_notebook(out_dir=None, job_name=None, config_name="default", config_path="../configs"):
    """
    从 Jupyter Notebook 启动训练。

    这个函数提供了一种在 Jupyter Notebook 环境中启动训练的便捷方式。
    它加载一个 Hydra 配置，并允许覆盖输出目录和作业名称。

    参数:
        out_dir (str, optional): 保存所有运行输出的目录。默认为 None。
        job_name (str, optional): 作业的名称。默认为 None。
        config_name (str, optional): 要使用的配置文件名称。默认为 "default"。
        config_path (str, optional): 配置文件的路径。默认为 "../configs"。
    """
